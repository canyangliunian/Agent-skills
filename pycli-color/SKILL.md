...
- 必须覆写 `HelpFormatter._format_action_invocation(self, action)` 以控制每个 action 的 option/metavar 渲染。
...
注意：你的环境中常见 `NO_COLOR=1` 全局存在；若要强制彩色，请用 `NO_COLOR= FORCE_COLOR=1 ...` 清空并强制启用。
